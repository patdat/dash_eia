import pandas as pd
import numpy as np
from eia_downloads.wps.mapping import production_mapping
from eia_downloads.wps.calculations import generate_additional_tickers
...
def third_pass(df):
    df = df.copy()
    mapping = pd.read_csv(config.get_lookup_path("wps", "09_csv_mapping.csv"))
...
def download_data():
    try:
        ...
        return pd.read_feather(str(config.get_data_path("wps", "eia_weekly_fast_download_psw09.feather"))), isAvailable
    ...
    if isAvailable:
        ...
        df.to_feather(str(config.get_data_path("wps", "eia_weekly_fast_download_psw09.feather")))
...
def main():
    ...
    if isAvailable:
        ...
        master = pd.read_feather(str(config.get_data_path("wps", "wps_gte_2015.feather")))
        ...
        df.reset_index(drop=True, inplace=True)
        df.to_feather(str(config.get_data_path("wps", "wps_gte_2015.feather")))
        pv = pivot_data(df)
        pv.reset_index(drop=True,inplace=True)
        pv.to_feather(str(config.get_data_path("wps", "wps_gte_2015_pivot.feather")))
    else:
        pv = pd.read_feather(str(config.get_data_path("wps", "wps_gte_2015_pivot.feather")))
