...
def get_release_date_from_soup(soup, category):
    ...
    mapping = {
        'steo': lambda s: s.find("strong", string="Release Date:").next_sibling,
    }
...
def updatecompiler():
    old = pd.read_csv(config.get_lookup_path("steo", "release_date_steo.csv"), parse_dates=['release_date'])
