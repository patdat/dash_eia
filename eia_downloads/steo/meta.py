...
def download_api():
    ...
    api_key = os.environ.get("EIA_API_KEY", "6EF7F9D5C3541865CEC21B793BAC4433")
    params = {
        "api_key": api_key,
        "frequency": "monthly",
        "data[0]": "value",
...
def create_csv(df):
    ...
    shorthand_replacement = pd.read_csv(config.get_lookup_path("steo", "unit_shorthand_steo.csv"))
...
def main():
    ...
    df = create_csv(df)
    meta = create_meta(df)
    meta.to_csv(config.get_lookup_path("steo", "metadata_steo.csv"), index=False)
