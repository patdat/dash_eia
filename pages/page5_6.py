...
from dash.dash_table.Format import Format, Group, Sign, Scheme
...
def generate_seasonality_table(padd_filter='US'):
    ...
    return dash_table.DataTable(
        ...
        columns=[
            ...
            {"name": "Avg Volume (Mbbl)", "id": "Avg Volume", "type": "numeric", "format": Format(scheme=Scheme.fixed, precision=1, group=Group.yes, group_delimiter=',')},
            {"name": "Seasonal Factor", "id": "Seasonal Factor", "type": "numeric", "format": Format(scheme=Scheme.fixed, precision=3)},
            {"name": "Deviation %", "id": "Deviation from Mean", "type": "numeric", "format": Format(scheme=Scheme.fixed, precision=1, sign=Sign.positive)}
        ],
