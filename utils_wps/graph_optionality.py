...
chart_seag = {**chart_button_style_base, "background-color": "black", "color": "white"}

chart_line = {**chart_button_style_base, "background-color": "black", "color": "white"}
...
toggler_on = {**button_style_base, "background-color": "black", "color": "white"}

toggler_off = {**button_style_base, "background-color": "black", "color": "white"}
...
off_button = {**small_button_style_base, "background-color": "white", "color": "grey"}
...
def checklist_header(
    ...
    html_container = html.Div(
        [
            ...
            dcc.Store(
                id=f"{seasonality_buttons}-state", data=True, storage_type="session"
            ),
            dcc.Store(
                id=f"{line_buttons_div}-state", data=True, storage_type="session"
            ),
            dcc.Store(
                id=f"{toggle_seag_range}-state", data=True, storage_type="session"
            ),
            ...
            dcc.Store(
                id=f"{btn_all}-state", data=True, storage_type="session"
            ),  # Default to 'all' active
            html.Div(
                [
                    html.Button(
                        id=chart_toggle,
                        children="Graph: Seasonality",
                        n_clicks=0,
                        style=chart_seag,
                    ),
                ],
            ...
            html.Div(
                [
                    html.Button(
                        id=toggle_seag_range,
                        children="Range: 2018-2023",
                        n_clicks=0,
                        style=toggler_on,
                    ),
                    html.Button(
                        id=toggle_2022, children="2022", n_clicks=0, style=button_2022
                    ),
                    html.Button(
                        id=toggle_2023, children="2023", n_clicks=0, style=button_2023
                    ),
                    html.Button(
                        id=toggle_2024, children="2024", n_clicks=0, style=button_2024
                    ),
                ],
                id=seasonality_buttons,
                style={"display": "block"},
            ),
            # CENTER THIS
            html.Div(
                [
                    html.Button(
                        id=btn_1m, children="-1m", n_clicks=0, style=line_buttons
                    ),
                    html.Button(
                        id=btn_6m, children="-6m", n_clicks=0, style=line_buttons
                    ),
                    html.Button(
                        id=btn_12m, children="-12m", n_clicks=0, style=line_buttons
                    ),
                    html.Button(
                        id=btn_36m, children="-36m", n_clicks=0, style=line_buttons
                    ),
                    html.Button(
                        id=btn_all, children="all", n_clicks=0, style=active_line_button
                    ),  # Default to 'all' active
                ],
                id=line_buttons_div,
                style={"display": "none"},
    ...
    @app.callback(
        [
            ...
            Output(seasonality_buttons, "style"),
            Output(line_buttons_div, "style"),
    ...
    @app.callback(
        [
            Output(toggle_seag_range, "style"),
            Output(toggle_seag_range, "children"),
    ...
    def toggle_range_button(n_clicks, current_text):
        ...
        return (
            (toggler_off, "Range: 2015-2019", False)
            if state
            else (toggler_on, "Range: 2018-2023", True)
        )
    ...
    @app.callback(
        [
            Output(btn_1m, "style"),
            Output(btn_6m, "style"),
            Output(btn_12m, "style"),
            Output(btn_36m, "style"),
            Output(btn_all, "style"),
            Output(f"{btn_1m}-state", "data"),
            Output(f"{btn_6m}-state", "data"),
            Output(f"{btn_12m}-state", "data"),
            Output(f"{btn_36m}-state", "data"),
            Output(f"{btn_all}-state", "data"),
        ],
        [
            Input(f"{btn_1m}", "n_clicks"),
            Input(f"{btn_6m}", "n_clicks"),
            Input(f"{btn_12m}", "n_clicks"),
            Input(f"{btn_36m}", "n_clicks"),
            Input(f"{btn_all}", "n_clicks"),
        ],
    )
    def toggle_line_buttons(
        btn_1m_clicks, btn_6m_clicks, btn_12m_clicks, btn_36m_clicks, btn_all_clicks
    ):
